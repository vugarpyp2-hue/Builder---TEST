...
## Follow-up hardening
- Export rendering iframe now runs without script execution permission (`sandbox="allow-same-origin"`), reducing risk while still allowing DOM/CSS inspection.
- CSS variable extraction now parses style blocks in addition to computed values, then binds style properties to variable IDs by resolved value matching.
