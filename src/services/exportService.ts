...
const classifyVariableType = (name: string, value: string): WebflowVariable['type'] => {
  if (name.includes('color') || value.startsWith('rgb') || value.startsWith('#')) return 'color';
...
const collectVariables = (doc: Document, view: Window): WebflowVariable[] => {
  const found = new Map<string, WebflowVariable>();
  ...
  for (const styleTag of Array.from(doc.querySelectorAll('style'))) {
    ...
    while ((match = regex.exec(css)) !== null) {
      ...
      if (!value) continue;
      found.set(name, {
        id: stableId('var', name),
        name,
        value,
        type: classifyVariableType(name, value),
      });
    }
  }
...
const parseHtmlToWebflowJson = async (html: string, artifact: Artifact): Promise<WebflowSchema> => {
  ...
  const variables = collectVariables(doc, iframe.contentWindow!);
  const variableByValue = new Map<string, string>();
  variables.forEach((variable) => {
    variableByValue.set(variable.value, variable.id);
  });
        ...
        const style: WebflowStyle = {
          ...
          tokenBindings: Object.fromEntries(
            Object.entries(properties)
              .filter(([, value]) => variableByValue.has(value))
              .map(([property, value]) => [property, variableByValue.get(value)!]),
          ),
